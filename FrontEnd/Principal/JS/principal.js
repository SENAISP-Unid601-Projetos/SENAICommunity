...
function highlightActiveSidebarItem() {
  ...
  menuItems.forEach((item) => {
    ...
    if (
      ...
      (currentPage === "" && itemHref === "principal.html")
    ) {
    ...
    if (
      currentPage === "buscar_amigos.html" &&
      itemHref === "buscar_amigos.html"
    ) {
      item.classList.add("active");
    }
  });
...
function createNotificationElement(notification) {
  ...
  // 6. NOTIFICAÇÕES DE EVENTOS (Início, Fim e Lembrete)
    ...
    // Lembrete genérico ou confirmação
  ...
  // 2. MENSAGEM PRIVADA (Novo)
    ...
    notification.tipo === "MENSAGEM_PRIVADA" ||
    notification.tipo === "NOVA_MENSAGEM"
  ) {
  ...
  // 3. MENSAGEM DE PROJETO / ATUALIZAÇÃO DE PROJETO (Novo)
    ...
    notification.tipo === "MENSAGEM_PROJETO" ||
    notification.tipo === "CONVITE_PROJETO" ||
    notification.tipo === "ATUALIZACAO_PROJETO"
  ) {
...
window.handleNotificationClick = function (event, notificationId, targetUrl) {
  ...
  axios
    .put(`${window.backendUrl}/api/notificacoes/${notificationId}/ler`)
...
function setupGlobalEventListeners() {
  ...
  if (notifIcon && notifIcon.parentNode) {
    ...
    globalElements.notificationsPanel = newIcon.querySelector(
      "#notifications-panel"
    );
    globalElements.notificationsList = newIcon.querySelector(
      "#notifications-list"
    );
    globalElements.notificationsBadge = newIcon.querySelector(
      "#notifications-badge"
    );
  ...
  if (
    globalElements.editProfileBtn &&
    globalElements.editProfileBtn.parentNode
  ) {
    const newProfileBtn = globalElements.editProfileBtn.cloneNode(true);
    globalElements.editProfileBtn.parentNode.replaceChild(
      newProfileBtn,
      globalElements.editProfileBtn
    );
  ...
  if (globalElements.logoutBtn && globalElements.logoutBtn.parentNode) {
    ...
    globalElements.logoutBtn.parentNode.replaceChild(
      newLogoutBtn,
      globalElements.logoutBtn
    );
  ...
  if (globalElements.cancelEditProfileBtn) {
    globalElements.cancelEditProfileBtn.addEventListener("click", () => {
      if (globalElements.editProfileModal)
  ...
  if (globalElements.cancelDeleteAccountBtn) {
    globalElements.cancelDeleteAccountBtn.addEventListener("click", () => {
      if (globalElements.deleteAccountModal)
...
window.createCommentElement = (comment, post, allComments) => {
  ...
  const isAuthor =
    window.currentUser &&
    String(autorIdDoComentario) === String(window.currentUser.id);
  const isPostOwner =
    window.currentUser &&
    String(autorIdDoPost) === String(window.currentUser.id);
  ...
  if (isAuthor || isPostOwner) {
    ...
    const safeContent = comment.conteudo
      ? comment.conteudo.replace(/'/g, "\\'")
      : "";
    ...
    optionsMenu = `
            ...
            <div class="options-menu" id="comment-menu-${
              comment.id
                ...
                ${
                  isAuthor
                    ? `<button onclick="window.openEditCommentModal(${comment.id}, '${safeContent}')"><i class="fas fa-pen"></i> Editar</button>`
                    : ""
                }
                ${
  ...
  if (comment.parentId && allComments) {
    ...
    if (parentComment && parentComment.parentId) {
      ...
      tagHtml = `<a href="perfil.html?id=${parentAuthorId}" class="reply-tag">@${
        comment.replyingToName || "Usuario"
      }</a>`;
    }
  }
                  ...
                  comment.destacado ? "destacado" : ""
                    ...
                    <a href="${profileLink}" class="comment-avatar-link">
                        <div class="comment-avatar">
                            <img src="${commentAuthorAvatar}" alt="Avatar" onerror="this.src='${
    window.defaultAvatarUrl
  }'">
                        </div>
                    </a>
...
window.renderCommentWithReplies = (
  comment,
  allComments,
  post,
  ...
  if (replies.length > 0 && !isAlreadyInReplyThread) {
    ...
    replies.forEach((reply) => {
      commentHtml += window.renderCommentWithReplies(
        reply,
        allComments,
        post,
        true
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (editCommentForm) {
    editCommentForm.addEventListener("submit", async (e) => {
      ...
      try {
        await axios.put(
          `${window.backendUrl}/comentarios/${commentId}`,
          { conteudo: content },
...
window.sendComment = (postId, parentId = null) => {
  ...
  if (window.stompClient?.connected && content) {
    ...
    window.showNotification(
      "Erro: Conexão perdida ou comentário vazio.",
      "error"
    );
  }
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    displayItems.forEach((url, index) => {
      ...
      mediaHtml += `
              <div class="post-media-item${moreClass}" 
                   onclick="window.openMediaViewer(${JSON.stringify(
                     mediaUrls
                   )}, ${index})">
          `;
    ...
    if (post.urlsMidia && post.urlsMidia.length > 0) {
      if (post.urlsMidia.length > 2) {
        ...
        const fullMediaUrl = url.startsWith("http")
          ? url
          : `${window.backendUrl}${url}`;
        const safeMediaArray = JSON.stringify(post.urlsMidia).replace(
          /"/g,
          "&quot;"
        );

        if (
          url.match(/\.(mp4|webm|mov|avi|mkv|flv|wmv|3gp|ogv|m3u8|ts|asf)$/i)
        ) {
  ...
  async function fetchAndReplacePost(postId) {
    ...
    if (oldPostElement) {
      ...
      const oldCommentsSection =
        oldPostElement.querySelector(".comments-section");
      ...
      const oldReplyContainers =
        oldPostElement.querySelectorAll(".comment-replies");
    ...
    try {
      ...
      if (wasCommentsVisible) {
        const newCommentsSection =
          newPostElement.querySelector(".comments-section");
      ...
      if (openReplyContainerIds.size > 0) {
        openReplyContainerIds.forEach((containerId) => {
          const newReplyContainer = newPostElement.querySelector(
            `#${containerId}`
          );
          if (newReplyContainer) {
            // Encontra o botão que controla este container
            const commentId = containerId.replace("replies-for-", "");
            const button = newPostElement.querySelector(
              `button[onclick*="window.toggleReplies(this, ${commentId})"]`
            );
