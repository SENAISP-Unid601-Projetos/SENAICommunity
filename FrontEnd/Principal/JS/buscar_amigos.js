document.addEventListener("DOMContentLoaded", () => {
    document.addEventListener('globalScriptsLoaded', (e) => {
        ...
        async function buscarUsuarios(nome) {
            ...
            try {
                const response = await window.axios.get(`${window.backendUrl}/usuarios/buscar`, {
                    params: { nome }
                });
        ...
        function renderizarResultados(usuarios) {
            ...
            usuarios.forEach(usuario => {
                ...
                const fotoUrl = usuario.fotoPerfil ? `${window.backendUrl}${usuario.fotoPerfil}` : window.defaultAvatarUrl;
        ...
        window.enviarSolicitacao = async (idSolicitado, buttonElement) => {
            ...
            try {
                ...
                window.showNotification('Não foi possível enviar a solicitação.', 'error');
