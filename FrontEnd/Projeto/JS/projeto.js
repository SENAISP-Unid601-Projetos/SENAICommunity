document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('globalScriptsLoaded', (e) => {
        ...
        // Pega o usuário global carregado pelo principal.js
        const currentUser = window.currentUser;

        // --- OBJETO DA PÁGINA DE PROJETOS ---
        // Agrupa toda a lógica específica desta página
        const ProjetosPage = {
            ...
            async init() {
                ...
                if (this.elements.connectionsCount) {
                    this.elements.connectionsCount.textContent = window.userFriends?.length || '0';
                }
            ...
            render() {
                ...
                projetosParaRenderizar.forEach(proj => {
                    ...
                    const imageUrl = proj.imagemUrl ? `${window.backendUrl}/projetos/imagens/${proj.imagemUrl}` : 'https://placehold.co/600x400/161b22/ffffff?text=Projeto';
                    
                    const membrosHtml = (proj.membros || []).map(membro => {
                        const avatarUrl = membro.usuarioFotoPerfil ? `${window.backendUrl}/api/arquivos/${membro.usuarioFotoPerfil}` : window.defaultAvatarUrl;
            ...
            handlers: {
                openModal() {
                    if (currentUser) {
                        ...
                        const avatarUrl = currentUser.urlFotoPerfil ? `${window.backendUrl}${currentUser.urlFotoPerfil}` : window.defaultAvatarUrl;
