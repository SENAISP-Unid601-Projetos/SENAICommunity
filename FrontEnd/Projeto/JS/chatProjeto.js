(() => {
  ...
  function renderGroupsList() {
    ...
    filteredGroups.forEach(g => {
      ...
      `;
      groupCard.addEventListener('click', function() { selectGroup(g.id); });
  ...
  function selectGroup(groupId) {
    ...
    renderChatHeader();
    renderChatMessages();
    if (chatInput && 'disabled' in chatInput) chatInput.disabled = false;
    if (chatSendBtn && 'disabled' in chatSendBtn) chatSendBtn.disabled = false;
  ...
  function renderChatHeader() {
    ...
    if ('src' in chatGroupAvatar && 'alt' in chatGroupAvatar) {
  ...
  if (groupSearchEl) {
    groupSearchEl.addEventListener('input', function () {
      let q = '';
      if (groupSearchEl instanceof HTMLInputElement || groupSearchEl instanceof HTMLTextAreaElement) {
        q = groupSearchEl.value.toLowerCase();
      }
  ...
  const savedThemeLocal = localStorage.getItem('theme') || 'dark';
